...
from managers.log_manager import get_date, log_erro
...
def send_to_s3(img_bytes: str, file_name: str):
    ...
    # ------- Upload da imagem -------  
        date = get_date()
        file_name = f"{date.strftime('%d-%m-%Y')}_{c.g_id_exec}_{file_name}"
        s3_client.upload_fileobj(screenshot_stream, c.a_bucket, file_name)
    
    
    except:
        log_erro(" S3 | Falha enviando print para o bucket")
